...
function submit_state() {
	...
	$.ajax({
		url : '/admin/stateDeliveryCompleted',
		type : 'POST',
		data : queryString,
		success : function(data) {
			if(data == 'success'){
				alert("선택 하신 주문의 상태가 '배송완료' 로 변경되었습니다.");
...
<section id="all">
	...
	<div>
		<form action="/admin/itSearch">
			...
			<button class="btn btn-outline-secondary my-2 my-sm-0" onclick="javascript:window.location = '/admin/inTransit'">검색 초기화</button>
		</form>
	...
	<div>
		<table id="myList" cellpadding="0" cellspacing="0" border="1">
	        ...
	        <tbody>
	    <c:forEach items="${inTransit}" var="transit">
		        <tr>
		        	<td><input type='checkbox' name='bco_ordernum' value='${transit.bco_ordernum}' onclick='checkSelectAll(event)'/></td>
		            <td>${transit.bco_ordernum}</td>
		            <td><fmt:formatDate value="${transit.bco_orderdate}" pattern="yyyy-MM-dd HH:mm"/></td>
		            <td>${transit.bcm_name}</td>
		            <td>${transit.bco_recipient}</td>
		            <td id="price${transit.bco_ordernum}">${transit.bco_totalprice}</td>
		            <td><a href="#" 
		            	onclick="javascript:openPop${transit.bco_ordernum}()">${transit.bco_order_name}</a>
		            </td>
		        </tr>
		        <script>
				    var money = $('#price${transit.bco_ordernum}').text();
				    var money2 = money.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				    $('#price${transit.bco_ordernum}').text(money2+"원");
				</script>
				<script>
					function openPop${transit.bco_ordernum}() {
		    	
						var queryString = 'bco_ordernum=${transit.bco_ordernum}';
	        ...
	        <tr>
	            <td colspan="7">
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="inTransit?page=1"> &lt;&lt; </a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="itSearch?page=1&searchType=${page.searchType}&searchWord=${page.searchWord}"> &lt;&lt; </a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="inTransit?page=${page.curPage-1}">&lt;</a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="itSearch?page=${page.curPage-1}&searchType=${page.searchType}&searchWord=${page.searchWord}">&lt;</a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	                ...
	                <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
	                    <c:choose>
	                        ...
	                        <c:otherwise>
	                        	<c:if test="${page.searchType == null }">
	                            	<a href="inTransit?page=${fEach}">${fEach}</a>&nbsp;
	                            </c:if>
	                            <c:if test="${page.searchType != null }">
	                            	<a href="itSearch?page=${fEach}&searchType=${page.searchType}&searchWord=${page.searchWord}">${fEach}</a>&nbsp;
	                            </c:if>
	                        </c:otherwise>
	                    </c:choose>
	                </c:forEach>
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="inTransit?page=${page.totalPage}">&gt;&gt;</a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="itSearch?page=${page.totalPage}&searchType=${page.searchType}&searchWord=${page.searchWord}">&gt;&gt;</a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	            </td>
    	...
    	<button onclick="submit_state()" class="btn btn-outline-secondary my-2 my-sm-0">선택한 주문 배송완료</button>
	</div>
</section>
</body>
</html>