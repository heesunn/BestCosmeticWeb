...
<section id="all">
	...
	<div>
		<form action="/admin/dcSearch">
			...
			<button class="btn btn-outline-secondary my-2 my-sm-0" onclick="javascript:window.location = '/admin/deliveryCompleted'">검색 초기화</button>
		</form>
		<p>
	...
	<div>
		<table id="myList" cellpadding="0" cellspacing="0" border="1">
	        ...
	        <tbody>
	    <c:forEach items="${deliveryCompleted}" var="completed">
		        <tr>
		            <td>${completed.bco_ordernum}</td>
		            <td><fmt:formatDate value="${completed.bco_orderdate}" pattern="yyyy-MM-dd HH:mm"/></td>
		            <td>${completed.bcm_name}</td>
		            <td>${completed.bco_recipient}</td>
		            <td id="price${completed.bco_ordernum}">${completed.bco_totalprice}</td>
		            <td><a href="#" 
		            	onclick="javascript:openPop${completed.bco_ordernum}()">${completed.bco_order_name}</a>
		            </td>
		        </tr>
		        <script>
				    var money = $('#price${completed.bco_ordernum}').text();
				    var money2 = money.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				    $('#price${completed.bco_ordernum}').text(money2+"원");
				</script>
				<script>
					function openPop${completed.bco_ordernum}() {
		    	
						var queryString = 'bco_ordernum=${completed.bco_ordernum}';
	        ...
	        <tr>
	            <td colspan="7">
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="deliveryCompleted?page=1"> &lt;&lt; </a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="dcSearch?page=1&searchType=${page.searchType}&searchWord=${page.searchWord}"> &lt;&lt; </a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="deliveryCompleted?page=${page.curPage-1}">&lt;</a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="dcSearch?page=${page.curPage-1}&searchType=${page.searchType}&searchWord=${page.searchWord}">&lt;</a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	                ...
	                <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
	                    <c:choose>
	                        ...
	                        <c:otherwise>
	                        	<c:if test="${page.searchType == null }">
	                            	<a href="deliveryCompleted?page=${fEach}">${fEach}</a>&nbsp;
	                            </c:if>
	                            <c:if test="${page.searchType != null }">
	                            	<a href="dcSearch?page=${fEach}&searchType=${page.searchType}&searchWord=${page.searchWord}">${fEach}</a>&nbsp;
	                            </c:if>
	                        </c:otherwise>
	                    </c:choose>
	                </c:forEach>
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                    	<c:if test="${page.searchType == null }">
	                        	<a href="deliveryCompleted?page=${page.totalPage}">&gt;&gt;</a>
	                        </c:if>
	                        <c:if test="${page.searchType != null }">
	                        	<a href="dcSearch?page=${page.totalPage}&searchType=${page.searchType}&searchWord=${page.searchWord}">&gt;&gt;</a>
	                        </c:if>
	                    </c:otherwise>
	                </c:choose>
	            </td>
